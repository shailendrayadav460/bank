...
import { 
  Heart, FileText, Calendar, LogOut, Menu, 
  Bell, HeartPulse, Sun, Moon, Search
} from 'lucide-react';
...
const PatientLayout = () => {
  ...
  const navItems = [
    { title: 'My Health',    icon: <Heart    size={20} />, path: '/patient' },
    { title: 'Records',      icon: <FileText size={20} />, path: '/patient/reports' },
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", display: 'flex', height: '100vh', overflow: 'hidden', backgroundColor: T.bg, color: T.text }}>
      ...
      <aside style={{ width: sidebarOpen ? 240 : 72, flexShrink: 0, backgroundColor: T.sidebar, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: T.shadow }}>
        ...
        <div style={{ padding: 24, display: 'flex', alignItems: 'center', gap: 10, minHeight: 80 }}>
          ...
          {sidebarOpen && <span style={{ fontSize: 17, fontWeight: 700, color: T.text, whiteSpace: 'nowrap' }}>My Health</span>}
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.title} to={item.path}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', fontSize: 14, fontWeight: 500, textDecoration: 'none', borderLeft: `3px solid ${active ? ACCENT : 'transparent'}`, backgroundColor: active ? (T.name === 'dark' ? 'rgba(16,185,129,0.12)' : 'rgba(16,185,129,0.07)') : 'transparent', color: active ? T.text : T.textSub, whiteSpace: 'nowrap' }}
        ...
        <div style={{ padding: 24, borderTop: `1px solid ${T.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name || 'Patient'}`} alt="avatar"
              style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, backgroundColor: T.primaryBg }} />
            {sidebarOpen && (
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: T.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name || 'Patient User'}</div>
                <div style={{ fontSize: 12, color: T.textSub }}>Health Profile</div>
              </div>
            )}
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <header style={{ height: 72, backgroundColor: T.card, borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', flexShrink: 0, boxShadow: T.shadow }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: T.text, margin: 0 }}>Patient Portal</h1>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ position: 'relative', width: 240 }}>
              <Search size={15} style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: T.textSub }} />
              <input type="text" placeholder="Search records..." style={{ width: '100%', backgroundColor: T.inputBg, border: `1px solid ${T.border}`, borderRadius: 6, padding: '7px 14px 7px 36px', fontSize: 14, color: T.text, outline: 'none' }} />
            </div>
